    ...
    for(i=0;i<data.length;i++){
        ...
        switch(data[i]){
            ...
            case TYPE_GRIDEYE: //Grideye data
                ...
                obj.data.grideye = [];
                for(var j = 0; j < 64; j++) {
                    obj.data.grideye[j] = ref + (data[1+i+j] / 10.0);
                }
                i += 64;
            break
            ...
            case TYPE_EXT_TEMP2: //External temp 2
                ...
                if(typeof obj.externalTemperature2 === "number") {
                    obj.data.externalTemperature2 = [obj.externalTemperature2];
                } 
                if(typeof obj.data.externalTemperature2 === "object") {
                    obj.data.externalTemperature2.push(temp / 10);
                } else {
            ...
            case TYPE_EXT_ANALOG_UV: //Load cell analog uV
                obj.data.analogUv = (data[i + 1] << 24) | (data[i + 2] << 16) | (data[i + 3] << 8) | (data[i + 4]);
                i += 4;
            break
